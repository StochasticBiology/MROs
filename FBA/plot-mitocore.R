...
d <- read.csv("MitoCore/Results/MAX_ATP-no-restriction.csv")

# Import data for restricted oxygen (10% of oxygen used at at no KO)
d2 <- read.csv("MitoCore/Results/MAX_ATP-oxygen-restriction.csv")
...
png(file = paste("MitoCore/Results/",filename,sep=""), width = 900*res, height = 500*res, res = 72*res)
p
dev.off()
