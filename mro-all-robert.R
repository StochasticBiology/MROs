...
#### Run HyperTraPS-CT (i.e., pass endtimes to HyperTraPS) on the Timetree tree
library(phytools)
library(parallel)
library(sybilSBML)
library(ggplot2)
...
sf = 2
png("mro-ct-outputs.png", width = 700*sf, height = 600*sf, res = 72*sf)
print(ggarrange(g.data, ggarrange(g.bub.1,g.bub.2,g.bub.3,nrow=3), nrow=1))
dev.off()

# check summary
sf = 2
png("mro-ct-convergence.png", width = 700*sf, height = 600*sf, res = 72*sf)
print(g.blurb)
dev.off()
...
sf = 2
png("mro-ct-summary.png", width=700*sf, height=600*sf, res=72*sf)
print(g.summary)
dev.off()
...
sf = 2
png("mro-ct-pathways.png", width=600*sf, height=600*sf, res=72*sf)
print(g.pathways)
dev.off()

#### Save all data
save.image("mro-all-save.RData")