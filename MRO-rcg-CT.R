...
#### Run HyperTraPS-CT on the Timetree tree

args = commandArgs(trailingOnly = T)

# test if there is at least one argument: if not, return an error
if (length(args)!=2) {
  stop("Need two arguments, a Phyllip or Newick tree and binary strings of loss patterns.\n", call.=FALSE)
}
...
# read data
#tree = read.tree("Data/mro-timetree-tree.nwk")
...
data.timetree = curate.tree(tree, df, losses=TRUE)

# wrapper function for HyperTraPS-CT analysis
parallel.ct.fn = function(seed, src.data, featurenames) {
  return(HyperTraPS(src.data$dests, initialstates = src.data$srcs,
		    endtimes = data.timetree$times,
...
parallelised.CT.runs <- mcmapply(parallel.ct.fn, seed=1:n.seed,
                               MoreArgs = list(src.data=data.timetree,
                                               featurenames=featurenames),
                               SIMPLIFY = FALSE,
                               mc.cores = min(detectCores(), n.seed))

g.data = plotHypercube.curated.tree(data.timetree, scale.fn=geom_blank(), names=TRUE)
g.blurb = plotHypercube.summary(parallelised.CT.runs[[1]]) + theme(legend.position="none")
g.bub.1 = plotHypercube.bubbles(parallelised.CT.runs[[1]]) + theme(legend.position="none")
g.bub.2 = plotHypercube.bubbles(parallelised.CT.runs[[2]]) + theme(legend.position="none")
g.bub.3 = plotHypercube.bubbles(parallelised.CT.runs[[3]]) + theme(legend.position="none")
g.sg.1 = plotHypercube.sampledgraph2(parallelised.CT.runs[[1]], 
...
tmp.ci = tmp.other = tmp.tca = tmp.c34 = parallelised.CT.runs[[1]]
tmp.ci$routes = r.ci
tmp.tca$routes = r.tca
tmp.c34$routes = r.c34
tmp.other$routes = r.other
g.pathways = ggarrange(plotHypercube.motifs(parallelised.CT.runs[[1]])+theme(legend.position="none"),
...
# compare outputs and check convergence
sf = 2
png("Results/HyperTraPS-CT/mro-discrete-convergence.png", width = 700*sf, height = 600*sf, res = 72*sf)
print(ggarrange(g.data, ggarrange(g.bub.1,g.bub.2,g.bub.3,nrow=3), nrow=1))
dev.off()

# check summary
sf = 2
png("Results/HyperTraPS-CT/mro-discrete-trace.png", width = 700*sf, height = 600*sf, res = 72*sf)
print(g.blurb)
dev.off()
...
sf = 2
png("Results/HyperTraPS-CT/mro-discrete-summary.png", width=700*sf, height=600*sf, res=72*sf)
print(g.summary)
dev.off()
...
tmp.ci = tmp.other = tmp.tca = tmp.c34 = parallelised.discrete.runs[[1]]
tmp.ci$routes = r.ci
tmp.tca$routes = r.tca
tmp.c34$routes = r.c34
tmp.other$routes = r.other
...
sf = 2
png("Results/HyperTraPS-CT/mro-discrete-pathways.png", width=600*sf, height=600*sf, res=72*sf)
print(g.pathways)
dev.off()
